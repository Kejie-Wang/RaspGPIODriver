...
int GPIOConfig()
{
	...
	if(GPIODirection(pinCLK, SYSFS_GPIO_DIR_OUT) == -1 ||
		GPIODirection(pinCS, SYSFS_GPIO_DIR_OUT) == -1 ||
		GPIODirection(pinDIN, SYSFS_GPIO_DIR_OUT) == -1)
	{
		return -1;
	}
}
...
{
	//export the gpio
	if(GPIOUnexport(pinCLK) == -1 ||
		GPIOUnexport(pinCS) == -1	||
		GPIOUnexport(pinDIN) == -1)
	{
		return -1;
	}
}
